...
def add_data_to_db(data, **database_login_info):
    for item in data:
        try:
            ...
            cur = con.cursor()
            print(f"""INSERT INTO cache.{item["symbol"].replace(".","")} values('{item["timestamp"]}',{round(item["open"],2)}, {round(item["high"],2)}, {round(item["low"],2)}, {round(item["close"],2)}, {item["volume"]}, '{item["symbol"]}', '{_hash}');""")
            cur.execute(
