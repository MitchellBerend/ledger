...
while True:
    ...
    if track_handler_lib.request_checker(data) == "delete":
        symbol = data[0].upper()
        check = track_handler_lib.check_profile_for_existing_data(data[0],**database_login_info)
        if check == True:
        ...
        track_handler_lib.insert_data_in_profile(symbol,close,amount,**database_login_info)
